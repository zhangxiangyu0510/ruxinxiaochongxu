...
Page({
  ...
  stepProcess(e: any) {
    ...
    if (_number == 3) {
      ...
      wx.redirectTo({
        url: '/packageRecommend/pages/firstAdjust/firstAdjust?isGuidance=true',
      })
      return
    }
    this.setData({
  ...
  goToAddGoods() {
    ...
    // wx.setStorageSync('addGoodsData', JSON.stringify([..._searchData, ..._goodsData]))
    wx.navigateTo({
      url: '/packageRecommend/pages/firstAdjust/firstAdjust',
    })
  ...
  getShopkeeperList(id: any) {
    ...
    util.request(api.shopkeeperList, _data,
      'get').then(function (res: any) {
        ...
        console.log('RRRRRRR', _goodsData)
      });
  ...
  searchReachBottom() {
    let that = this
    console.log(this.data.searchTotal, this.data.searchGoodsData.length, '@@@@@', this.data.searchTotal >= this.data.searchGoodsData.length)
  ...
  recommendationList() {
    ...
    util.request(api.recommendationList, { shop_id: _userInfo.shop ? _userInfo.shop.id : 30 }).then((res: any) => {
      ...
      if (res.data.searchShopProductDtos && res.data.searchShopProductDtos.length > 0) {
        ...
        // wx.setStorageSync('addGoodsData', arr)

      }

      // if(res.data.firstRecommendation){
   ...
   */
  onShow() {
    this.setData({ stepProcessNumber: 1 })
    ...
    util.request(api.isTipsGuide, { type: 1, partner_id: partnerId },
      'get').then(function (res: any) {
        ...
        if (res.data) {
          ...
          _this.setData({ stepProcessNumber: 0 })
