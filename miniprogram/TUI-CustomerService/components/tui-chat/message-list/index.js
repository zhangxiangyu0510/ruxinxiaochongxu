...
Component({
  ...
  pageLifetimes: {
    show: function () {
        this.changeColor('my_header_avater',app.globalData.imageUrl+'/userCenterIcon/defaultMyIcon.svg', app.globalData.themeColor, 'stroke')
    }
  },
  ...
  methods: {
            ...
            changeColor(name, url, color = '#EBEBEB', type = 'fill') {
                svgColor(url, color, type).then((res) => {
    ...
    // 撤回消息
    revokeMessage() {
        console.log('imresopnse====',this.data.selectedMessage);
      wx.$TUIKit.revokeMessage(this.data.selectedMessage)
        .then((imResponse) => {
          ...
          if (imResponse.data.message.from === _userInfo.partner.uid) {
        ...
        .catch((imError) => {
          let titleValue = imError.code === 20022 ? '消息发送中，无法撤回' : imError.code === 20023 ? '该消息已被撤回' : '超过2分钟消息不支持撤回'
          wx.showToast({
            title: titleValue,
            duration: 800,
            icon: 'none',
    ...
    $onMessageRevoked(event) {
        ...
        console.log('userID===',this.data.conversation);
        // _userInfo.partner.uid
      if (event.data[0].from == this.data.conversation.userProfile.userID) {
        this.setData({
