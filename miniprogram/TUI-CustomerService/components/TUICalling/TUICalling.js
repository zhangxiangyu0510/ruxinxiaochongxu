  ...
  properties: {
    config: {
      ...
      value: {
        ...
        userID: wx.getStorageSync('currentUserInfo').uid,
    ...
    async init() {
      try {
        const res = await wx.$TRTCCalling.login({
          userID: this.data.config.userID ||wx.getStorageSync('currentUserInfo').uid,
          userSig: this.data.config.userSig||wx.getStorageSync('userSign'),
        });
